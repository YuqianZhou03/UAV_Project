/**
 * FL-UAV dashboard: dataset pipeline story, workflow sim.
 */
...
function renderWorkflowResult(data, langZh) {
  ...
  if (!data.ok) {
    ...
    if (data.traceback) {
      const host = document.getElementById("workflowSteps");
      if (host) host.innerHTML = renderTracebackPanel(data.traceback, langZh);
    } else {
      const host = document.getElementById("workflowSteps");
...
async function runWorkflowStage(action, langZh) {
  ...
  try {
    ...
    if (action === "start_training") {
      ...
      if (st) {
        st.classList.remove("is-error");
        st.textContent = data.message || (langZh ? "步骤执行完成。" : "Step finished.");
      }
      ...
      if (host) {
        if (action === "start_server") {
          ...
          host.querySelector("#wf-drones")?.classList.add("wf-phase--active");
          host.querySelectorAll(".wf-drone").forEach((d) => d.classList.add("wf-drone--scan"));
          setWorkflowCaption(langZh, langZh ? "② 三机终端已启动，准备训练…" : "② Three UAV terminals started, ready for training…");
        }
      }
    }
  } catch (e) {
...
async function init() {
  try {
    ...
    if (b1) b1.addEventListener("click", () => runWorkflowStage("start_server", true));
    const b2 = document.getElementById("workflowStartClientsBtn");
    if (b2) b2.addEventListener("click", () => runWorkflowStage("start_clients", true));
    const b3 = document.getElementById("workflowTrainBtn");
    if (b3) b3.addEventListener("click", () => runWorkflowStage("start_training", true));
  } catch (e) {
